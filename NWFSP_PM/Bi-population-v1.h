...
/*
	Compared with Bipopulation_memetic, Bipopulation_memetic_version1 change its q-learning mechanism to

	The q-table is updated by the change of BSN and WSM of best individual in population in each generation, which leads to an update of q-table per generation

	The reward is set to the DeltaPT of the best individual
*/
...
class Bipopulation_memetic_version1{
	...
	std::vector<int> last_best_BSN;
	std::vector<int> last_best_WSM;
	int last_best_PT;
public:
	std::vector<int> Best;
	/*
	...
	*/
	...
	Bipopulation_memetic_version1(PARAMETERS::Params _param, std::string _str)
	...
	void run() {
		...
		logger_BSN << last_best_BSN ;
		logger_WSM << last_best_WSM ;
	...
	void framework(POP<Solution_Type>& pop) {
		...
		last_best_BSN.push_back(tmp->SI.bad_scenario_num);
		last_best_WSM.push_back(tmp->SI.worst_scenario_makespan);
		...
		for (int g = 0; g < param.max_gen; g++) {
			...
			if (g == 0) {
				...
				state = LS_Version1::get_state(last_best_BSN,last_best_WSM);
			...
			LS_Version1::Update_Qtable_Version1(last_best_BSN, last_best_WSM, q_table, state, scenario_idx, pop, last_best_PT, param,g+1);
			//--------------------------------------------------
