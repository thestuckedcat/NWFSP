	...
	private:
		/*
			...
			THE FOLLOWING INFO is not dealed with:
				...
				2. SI
		*/
		void NEH(std::vector<int>& current_seq, int job_idx, std::vector<std::vector<int>>& pt) {
		...
		Solution_type NEH_Generate(std::vector<std::vector<int>> &pt) {
			int job_num = pt.size();
			...
			{
				NEH(current_sequence, sum_indicies[i].first, pt);
			}
