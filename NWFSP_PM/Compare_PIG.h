...
/*
	1. Retained the Pairwise IG search structure from the original paper. In the original paper, both pair-wise selections kept the solution with the better makespan. Here, we made some modifications:
		Modification 1: After selecting a pair, the first inserted job retains the one with the best makespan under a randomly selected scenario.
		Modification 2: When inserting the second job, the solution with the optimal PT value is retained.

	2. Experiments showed that directly replacing the original algorithm's local search with the one in this paper resulted in extremely poor performance.
	   Similarly, using IG as the local search while keeping GA for global search also performed poorly.
	   Therefore, we retained the global and local search mechanisms from my original algorithm but incorporated IG search probabilistically after global search
	   to observe the impact of IG on the algorithm's performance.
*/
...
public:
	...
	*/
	...
	PIG(PARAMETERS::Params _param, std::string _str)
	...
	void framework(POP<Solution_Type>& pop) {
		...
		for (int g = 0; g <= param.max_gen; g++) {
			...
			GLOBAL_SEARCH::Pairwise_IG<POP, Solution_Type>()(pop,chosen_solution_index,param,scenario_idx);
