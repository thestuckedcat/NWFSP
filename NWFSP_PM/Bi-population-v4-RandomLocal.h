...
/*
	Compared with Bipopulation_memetic_version3, the single scenario neighbor structure is changed to randomly swap 
*/
...
class Bipopulation_memetic_version4 {
...
public:
	...
	*/
	...
	Bipopulation_memetic_version4(PARAMETERS::Params _param, std::string _str)
	...
	void framework(POP<Solution_Type>& pop) {
		...
		for (int g = 0; g <= param.max_gen; g++) {
			...
			if (g == 0) {
				...
				if (param.epsilon < 0)
				{
					if (epsilon > ((float)g / param.max_gen - 0.1))
					{
			...
			LS_Version4::CRITICAL_PATH_version4<POP, Solution_Type>()(pop, chosen_solution_index, param, scenario_idx);
			...
			long long best = std::min_element(pop.population.begin(), pop.population.end(), [](auto& a, auto& b) {
				return a.SI.Penalty_of_TBS < b.SI.Penalty_of_TBS;
				})->SI.Penalty_of_TBS;
			...
			if (g % 500 == 0) {
				logger_generationRelated << g << "\t" << best << "\ttime\t" << total_time << std::endl;
			}
