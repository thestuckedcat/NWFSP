...
/*
	Compared with Bipopulation_memetic_version3, single scenario local search is replaced as random VNS_neighbor structure
*/
	...
	void framework(POP<Solution_Type>& pop) {
		...
		for (int g = 0; g <= param.max_gen; g++) {
			...
			if (g == 0) {
				...
				if (param.epsilon < 0)
				{
					if (epsilon > ((float)g / param.max_gen - 0.2))
					{
			...
			total_time += elapsed.count() * param.machine_num / (5*traverse_time);
